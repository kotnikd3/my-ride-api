...
if [ "$1" = "local" ]; then
    uvicorn --host 0.0.0.0 --port $PORT api.infrastructure.routes:app
elif [ "$1" = "prod" ]; then
    ...
    uvicorn --host 0.0.0.0 --port $PORT --workers ${NUM_OF_UVICORN_WORKERS:-1} --log-level ${LOG_LEVEL:-warning} api.infrastructure.routes:app
elif [ "$1" = "test" ]; then
    # Clean the code
    if [ "$2" = "local" ]; then
      isort .
